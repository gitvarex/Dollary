...
class RentGUI:
    def __init__(self, return_to_home, rent_puncak, properties1, properties2, properties3):
        """
        Initialize the RentGUI class.

        Parameters:
            return_to_home (function): Function to return to the home screen.
            rent_puncak (function): Function to handle renting Puncak Banyan property.
    ...
    def book_room(self):
        ...
        family_members = self.family_members_entry.get()  # Getting family members from entry field

        if name and phone and email and card_info and duration and rental_type and family_members.isdigit():  # Checking if all fields are filled
            try:
                ...
                if 2 <= int(family_members) <= 4:  # Checking if family members are between 2 and 4
                ...
                if total_price > 0:  # Checking if total price is valid
                    for prop_list in [self.properties1, self.properties2, self.properties3]:  # Iterating over property lists
                        for property_info in prop_list:  # Iterating over properties in each list
                            if property_info["name"] == "Puncak Banyan":  # Checking if property name matches
